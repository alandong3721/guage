...
{
    public class SetPFIPEDepotRateForUserDAO
    {
        /// <summary>
        /// 获取用户 邮费类
        /// </summary>
        ...
        public UserRate getUserRate(string name)
        {
            ...
            string sql = "select * from tb_set_user_pf_ipe_depot_rate where username='" + name + "'";
        ...
        public int setRateForUser(string username, string ratetype, string setername)
        {
            ...
            string str = "select * from tb_set_user_pf_ipe_depot_rate where username='" + username + "'";
            ...
            {
                //该用户已经存在，即为更新操作
                string sql = "update tb_set_user_pf_ipe_depot_rate set ratetype='" + ratetype + "',setername='" + setername + "',settime='" + DateTime.Now.ToString() + "' where username='" + username + "'";
            ...
            {
                string sql = "insert into tb_set_user_pf_ipe_depot_rate values('" + username + "','" + ratetype + "','" + setername + "','" + DateTime.Now.ToString() + "')";
        ...
        public bool deleteUserRate(string username)
        {
            ...
            string sql = "delete from tb_set_user_pf_ipe_depot_rate where username='" + username + "'";
            if (new DBOperateCommon().excuteNoQuery(sql))
            {
                flag = true;
            }
        ...
        public DataTable getUserRate(int pageNow, int pageSize)
        {
            ...
            string sql = "select top " + pageSize + " * from tb_set_user_pf_ipe_depot_rate where username not in(select top " + (pageNow - 1) * pageSize + " username from tb_set_user_pf_ipe_depot_rate)";
        ...
        {
            int count = 0;
            string sql = "select count(*) from tb_set_user_pf_ipe_depot_rate";
            DataTable table = new DBOperateCommon().excuteQuery(sql);
            if (table.Rows.Count > 0)
            {
