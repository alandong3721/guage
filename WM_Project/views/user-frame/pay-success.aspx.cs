            ...
            if (!IsPostBack)
            {
                ...
                if (Session["name"].ToString() != null && code != null)
                {
                    if (new UserDAO().isCodeRight(Session["name"].ToString(), code))
                    {
                        ...
                        //    ArrayList local_array = new LocalPackageDAO();
                        //    CollectionPlusLabel collectionplus = new CollectionPlusLabel(, "18888881", "", "06/01/2015");
                        //collectionplus.makeCollectionPlusLabel(Server.MapPath("~\\views\\pdf\\local\\"+local_order_array));
                        }
